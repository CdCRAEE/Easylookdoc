# easylook_verticalnav_integrated_fix.py
# EasyLook.DOC — Vertical Menu (under logo) + Brand Buttons (no red)
# Integrated: Extraction (Step 1) + Chat (Step 2) with streaming + typing + smart autoscroll
...
.menu-card {{ background:#fff; border:1px solid #e6eaf0; border-radius:12px; padding:16px; }}
.logo-title {{ font-weight:900; color:{PRIMARY}; font-size:22px; line-height:1; margin-bottom: 6px; }}
.logo-sub {{ color:{ACCENT}; font-weight:900; }}
.logo-dot {{ color:{SECOND}; font-weight:900; }}
.nav-title {{ font-size:12px; letter-spacing:.8px; color:#667; text-transform:uppercase; margin:8px 0 6px; }}

/* Vertical radio nav */
.nav-radio .stRadio > div {{ display:flex; flex-direction:column; gap:6px; }}
.nav-radio label {{ width:100%; }}
/* make options look like clean items */
div[data-baseweb="radio"] label p {{ font-weight:600; color:#223; }}

/* Panel on right */
.panel {{ background:#fff; border:1px solid #e6eaf0; border-radius:12px; padding:18px; }}
.section-title {{ font-size:22px; font-weight:800; color:#1f2b3a; }}

/* Force brand buttons (override any theme red) */
.stButton>button {{ 
...
/* Per-Button wrappers */
.btn-accent button {{ background-color: var(--brand-accent) !important; color:#1b1b1b !important; border:1px solid #e2e2e2 !important; }}
.btn-outline button {{ background:#fff !important; color:#1f3a56 !important; border:2px solid #bcd0e5 !important; }}

.stTextInput>div>div>input {{ background:#f5f7fb; }}
/* Chat bubbles */
...
# ---- Utility to class-style Streamlit buttons ----
from contextlib import contextmanager
@contextmanager
def button_class(cls: str):
...
with left:
    ...
    shown=False
    try:
        st.image("images/Nuovo_Logo.png", width=180); shown=True
    ...
    ss["nav"]=st.radio("Navigazione",["Documenti","Estrazione","Chat","Cronologia","Impostazioni"],
                       ...
                       label_visibility="collapsed",key="nav_vertical")
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
with right:
    if ss["nav"] in ("Documenti","Estrazione"):
        ...
        st.markdown('<div class="section-title">Estrazione documento</div>', unsafe_allow_html=True)
        ...
        if not HAVE_FORMRECOGNIZER:
            ...
            with c1, button_class("btn-accent"):  # giallo brand
                read_clicked = st.button("🔎 Leggi documento", use_container_width=True)
            with c2, button_class("btn-outline"):
                ...
                if not (AZURE_DOCINT_ENDPOINT and (AZURE_DOCINT_KEY or os.getenv("AZURE_TENANT_ID")) and AZURE_BLOB_CONTAINER_SAS_URL and file_name):
                    ...
                    try:
                        ...
                        if full_text:
                            ...
                            st.text_area("Anteprima (~4000 caratteri):", full_text[:4000], height=220)
                            ss["document_text"]=full_text; ss["chat_history"]=[]; ss["doc_ready"]=True
                            ss["nav"]="Chat"; st.rerun()
                        else:
    ...
    if ss["nav"] == "Chat":
        ...
        if ss.get("doc_ready", False):
            ...
            if user_prompt:
                ...
                CONTEXT_CHAR_LIMIT=12000; SYS="Sei un assistente che risponde SOLO sulla base del documento fornito."; TRUNC="(---Documento troncato - mostra l'ultima parte---)\n"
                ...
                try:
                    ...
                    render_chat(chat_ph, ss["chat_history"], show_typing=False); st.error(f"❌ Errore nella chiamata API: {api_err}")
            col1, col2 = st.columns([1,1])
            with col1, button_class("btn-outline"):
                if st.button("Reset chat", use_container_width=True): ss["chat_history"]=[]; st.rerun()
            with col2: st.caption("Sessione locale al browser")
        else:
