...
.nav-item.active button[kind="secondary"]{
  background:#dbeafe !important; /* selezione */
...
with left:
    ...
    # NAV senza "pallini": tre pulsanti stile menu
    nav_labels = [("📤 Origine", "Leggi documento"),
                  ("💬 Chat", "Chat"),
                  ("🕒 Cronologia", "Cronologia")]
    for label, value in nav_labels:
        # wrapper per stato attivo
...
with right:
    ...
    # =================== CHAT ===================
        ...
        for m in ss['chat_history']:
            ...
            meta = m.get('ts', '')
            html_block = ("<div class='msg-row'><div class='msg %s'>%s"
                          "<div class='meta'>%s</div></div></div>") % (role_class, body, meta)
                ...
                try:
                    if search_client:
                        ...
                        results = search_client.search(
                            ...
                            filter=flt,
                            top=5,
                            query_type="simple"  # se abiliti Semantic: query_type="semantic", semantic_configuration_name="default"
